...
import { sanitizeUserData } from '../middleware/userSanitizer';
import { getUserWithDefaults, getUserForIdCardOperations } from '../services/userServices';
...
const convertToMySQLDate = (dateString: string): string | null => {
...
const convertToDisplayDate = (mysqlDate: any): string | null => {
...
// Create new ID card
router.post('/', authenticateToken, sanitizeUserData, requireRole(['admin', 'staff']), async (req: AuthRequest, res: Response) => {
  try {
    // Use service for business logic validation
    const businessUser = getUserForIdCardOperations(req.user);
    ...
    const [templates] = await pool.execute(
      `SELECT t.*, t.fields as template_fields 
       FROM templates t 
    ...
    const missingFields = [];
    if (requiredFields.includes('name') && !name) missingFields.push('name');
    if (requiredFields.includes('id_no') && !idNo) missingFields.push('id_no');
    if (requiredFields.includes('photo') && !photoUrl) missingFields.push('photo');

    // Only validate these if they are in requiredFields
    if (requiredFields.includes('mobile') && !mobile) missingFields.push('mobile');
    if (requiredFields.includes('dob') && !dob) missingFields.push('dob');
    if (requiredFields.includes('blood_group') && !bloodGroup) missingFields.push('blood_group');
    if (requiredFields.includes('email') && !email) missingFields.push('email');
    if (requiredFields.includes('address') && !address) missingFields.push('address');
    if (requiredFields.includes('father_name') && !fatherName) missingFields.push('father_name');
    if (requiredFields.includes('mother_name') && !motherName) missingFields.push('mother_name');
    ...
    const [existingCount] = await pool.execute(
      'SELECT COUNT(*) as count FROM id_cards WHERE created_by = ?',
      [req.user.id]
    );
    
    const currentCount = (existingCount as any[])[0].count;
    ...
    const [existingCards] = await pool.execute(
      'SELECT id FROM id_cards WHERE id_no = ? AND organization_id = ?',
      [idNo, organizationId]
    );
    ...
    const idCardId = (result as any).insertId;

    // Get the created ID card with template details
    const [idCards] = await pool.execute(
...
router.get('/', authenticateToken, sanitizeUserData, async (req: AuthRequest, res: Response) => {
  try {
    // Use service for consistent user data
    const safeUser = getUserWithDefaults(req.user);
    ...
    const [idCards] = await pool.execute(query, params);
    
    // SAFE JSON PARSING for all ID cards - FIXED
    const idCardsWithSafeFields = (idCards as any[]).map(card => ({
      ...card,
      // Convert dates back to DD/MM/YYYY for frontend
...
// Get ID card by ID
router.get('/:id', authenticateToken, sanitizeUserData, async (req: AuthRequest, res: Response) => {
  try {
    const idCardId = req.params.id;
    const safeUser = getUserWithDefaults(req.user);

    let query = '';
    let params: any[] = [idCardId];
    ...
    const [idCards] = await pool.execute(query, params);
    const idCardArray = idCards as any[];
...
router.patch('/:id/status', authenticateToken, sanitizeUserData, requireRole(['admin']), async (req: AuthRequest, res: Response) => {
  try {
    const idCardId = req.params.id;
    ...
    const [result] = await pool.execute(
      'UPDATE id_cards SET status = ? WHERE id = ? AND organization_id = ?',
      [status, idCardId, organizationId]
    );
...
router.get('/department/:department', authenticateToken, sanitizeUserData, requireRole(['admin']), async (req: AuthRequest, res: Response) => {
  try {
    ...
    const [idCards] = await pool.execute(
      `SELECT ic.*, t.name as template_name, u.name as creator_name
       FROM id_cards ic
       LEFT JOIN templates t ON ic.template_id = t.id
       LEFT JOIN users u ON ic.created_by = u.id
...
router.get('/my/cards', authenticateToken, sanitizeUserData, async (req: AuthRequest, res: Response) => {
  try {
    const safeUser = getUserWithDefaults(req.user);
